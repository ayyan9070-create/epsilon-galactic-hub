    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry, i) => {
          if (entry.isIntersecting && !visibleElements.includes(i)) {
    ...
    <div className="about-page text-white overflow-hidden">
      ...
      <section className="hero-section relative min-h-screen flex flex-col overflow-hidden">
        ...
        <main className="flex-grow relative flex flex-col items-center justify-center text-center z-20 px-6 -mt-16">
          <img
            ...
            className="mb-4 w-48 fade-up"
            ref={(el) => (elementsRef.current[0] = el)}
            alt="Badge"
          />
          <h1 className="leading-none fade-up" ref={(el) => (elementsRef.current[1] = el)}>
          ...
          <a className="block mt-10 hover:scale-105 transition-all">
            <img src="/img/hero-button.svg" className="w-[120px] h-[118px]" alt="Scroll" />
          </a>
        </main>
      </section>
        ...
        <div className="marquee-container">
          <div className="marquee-text">
            {marqueeText.map((text, i) => (
              <span key={i} className="text-4xl font-bold text-gradient px-6 tracking-widest">
                {text}
              </span>
            ))}
          </div>
      ...
      <section className="relative w-full py-24 px-6">
        <div className="max-w-[1300px] mx-auto text-center">
          ...
          <h2 className="font-clash-display text-6xl text-gradient mt-4 fade-up" ref={(el) => (elementsRef.current[2] = el)}>
            WHAT TO EXPECT
          </h2>
          <img
            src="/img/expect.svg"
            className="w-full max-w-[1300px] mx-auto mt-12 fade-up"
      ...
      <section className="relative py-24 overflow-hidden">
        ...
        <div className="relative max-w-[1300px] mx-auto px-6">
          <img src="/img/modules-badge.svg" className="mx-auto fade-up" ref={(el) => (elementsRef.current[4] = el)} />
          <h2 className="font-clash-display text-6xl text-gradient text-center mt-4 fade-up" ref={(el) => (elementsRef.current[5] = el)}>
            EPSILON VI MODULES
          </h2>
        </div>
      </section>
      ...
      <div className="container mx-auto px-4 py-20">
        {/* ABOUT HEADER */}
        <div className="text-center mb-20 fade-up" ref={(el) => (elementsRef.current[6] = el)}>
        ...
        {/* COUNTERS */}
        <div className="flex flex-col md:flex-row items-center justify-center gap-16 mb-20 fade-up" ref={(el) => (elementsRef.current[7] = el)}>
        ...
        {/* GALLERY */}
        <div className="mb-20 fade-up" ref={(el) => (elementsRef.current[8] = el)}>
        ...
        {/* EXECUTIVE BODY */}
        <div className="mb-20 fade-up" ref={(el) => (elementsRef.current[9] = el)}>
          <h2 className="text-4xl font-bold text-center mb-12 text-glow-purple">
            Executive Body
          </h2>
          <div className="space-y-6 max-w-4xl mx-auto">
            {executiveBody.map((member, index) => (
              <div key={index} className="p-8 frosted-glass rounded-xl border border-white/10">
                <div className="flex flex-col md:flex-row items-center gap-6">
                  ...
                  <div className="flex-1 text-center md:text-left">
                    <h3 className="text-4xl font-black text-white italic">{member.name}</h3>
                    <p className="text-purple-400 font-bold text-lg mb-3">{member.role}</p>
        ...
        {/* EXECUTIVE COUNCIL */}
        <div className="mb-20 fade-up" ref={(el) => (elementsRef.current[10] = el)}>
          <h2 className="text-4xl font-bold text-center mb-12 text-glow-purple">
            Executive Council
          </h2>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 max-w-6xl mx-auto">
            {executiveCouncil.map((member, index) => (
              <div key={index} className="p-6 frosted-glass rounded-xl border border-white/10 text-center">
                <img
                  src={member.image}
        ...
        {/* MISSION + VISION */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-20 fade-up" ref={(el) => (elementsRef.current[11] = el)}>
          <div className="p-8 frosted-glass rounded-xl border border-white/10">
            <Target className="w-12 h-12 text-purple-400 mb-4" />
            <h3 className="text-2xl font-bold mb-4 text-white">Our Mission</h3>
            <p className="text-white/80">To inspire and empower the next generation of scientists…</p>
          </div>
          <div className="p-8 frosted-glass rounded-xl border border-white/10">
            <Zap className="w-12 h-12 text-pink-400 mb-4" />
            <h3 className="text-2xl font-bold mb-4 text-white">Our Vision</h3>
            <p className="text-white/80">To establish Epsilon as the premier platform for STEM excellence…</p>
          </div>
        </div>

        {/* WHY PARTICIPATE */}
        <div className="text-center max-w-3xl mx-auto mb-20 fade-up" ref={(el) => (elementsRef.current[12] = el)}>
